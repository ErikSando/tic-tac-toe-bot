// In this file, the search algorithm is slightly changed so that the bot will choose the fastest path to winning
...
int MinMax(Grid& grid, int move, int side_to_move, int depth) {
    grid.MakeMove(move);

    if (grid.CheckVictory()) {
        grid.TakeMove(move);
        return grid.side == side_to_move ? (10 - depth) : -(10 - depth);
    }
    ...
    for (int i = 0; i < n_moves; i++) {
        int move_outcome = MinMax(grid, list->moves[i], side_to_move, depth + 1);
...
int Search(Grid& grid) {
    ...
    int best_move = -1, best_outcome = 0;

    int side_to_move = grid.side;

    for (int i = 0; i < list->length; i++) {
        int move = list->moves[i];
        int outcome = MinMax(grid, move, side_to_move, 1);
